"""
Django management command to import SAV calendar data from JSON file.
Usage: python manage.py import_sav_data [--file path/to/Sav.json] [--clear]
"""
import json
import os
...
class Command(BaseCommand):
    help = 'Import SAV calendar bookings from JSON file into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--file',
            type=str,
            default='data/Sav.json',
            help='Path to the JSON file (default: data/Sav.json)'
        )
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear all existing SAV calendar bookings before importing'
        )
    ...
    def handle(self, *args, **options):
        ...
        if clear_existing:
            deleted_count = Booking.objects.filter(calendar_id='calendar2').delete()[0]
            self.stdout.write(self.style.WARNING(f'Cleared {deleted_count} existing SAV calendar bookings'))
        
        # Import bookings
        calendar_id = 'calendar2'  # SAV calendar
        created_count = 0
        updated_count = 0
        skipped_count = 0
        error_count = 0
        ...
        for idx, booking_data in enumerate(bookings_data, 1):
            try:
                ...
                booking_data_mapped = {
                    'calendar_id': calendar_id,
                    'booking_date': booking_date,
                    'booking_time': booking_data.get('time', ''),  # Required for SAV
                    'client_name': booking_data.get('name', ''),
                    'client_phone': booking_data.get('phone', ''),
